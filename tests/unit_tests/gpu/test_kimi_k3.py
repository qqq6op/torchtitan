...
class TestKimiK3(unittest.TestCase):
    def test_preprocess_builds_token_aligned_vision_bank_indices(self):
        ...
        ContextParallelTransform(
            inner_attention={InnerKDA.Config: ContextParallelInnerKDA}
        ...
        cp_mesh = SimpleNamespace(
            ...
            size=lambda _axis=0: 2,
        )
        routing = object()
        shard_input_keys: set[str] = set()

        def shard_inputs(
            _load_balancer: ContextParallelLoadBalancer,
            batch: dict[str, object],
        ) -> dict[str, object]:
            shard_input_keys.update(batch)
            return batch
        ...
        with (
            ...
            inputs, labels, kwargs = model.preprocess_inputs(
                ...
                parallelism=ParallelismConfig(
                    ...
                    context_parallel_load_balancer=HeadTailLoadBalancer.Config(),
                ),
            )
        ...
        self.assertIs(kwargs["attention_masks"]["kda"].cp_routing, routing)
        ...
        self.assertNotIn("kda_cp_routing", shard_input_keys)
