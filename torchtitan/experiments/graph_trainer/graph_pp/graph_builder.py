...
@dataclasses.dataclass(slots=True)
class GraphTrainerJointStageGraphs(GraphPPJointStageGraphs):
    ...
    def __post_init__(self) -> None:
        self._run = run_traced(self.traced, module=self.module)
...
def _build_joint_stage_graph(
    ...
    *,
    ...
    if compile_config.enable_passes:
        pipeline_fn = PASS_PIPELINE_REGISTRY.get(compile_config.pass_pipeline)
        if pipeline_fn is None:
            if pass_config.training.disable_cuda_graphs:
...
def _build_stage_graphs(
    ...
    *,
    ...
    if fsdp_bw.reduction_node_names:
        ...
        # The split already constructed this GraphModule. Recompile after its
        # final graph mutation so forward reflects the updated graph.
